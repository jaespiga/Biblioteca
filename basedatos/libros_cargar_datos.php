...
leerDatosLibros($datos_entrada);

function leerDatosLibros($datos_entrada) { 
...
<?php  
    ...
    while($fila = $resultados->fetch_assoc()) { 
        ...
        if ($res[1] == "filtro") {
            ...
            if ($res[2] !== "" 
            && $res[2] !== $fila['cGR03_Autor']) {
                $ind_tratar = 1; 
            }
            if ($res[3] !== "0" 
            && $res[3] !== $fila['cGR03_Editorial']) {
                $ind_tratar = 1; 
            }

            if ($res[4] !== $fecha_minima
            &&  $res[5] !== $fecha_maxima){
                if ($fila['cGR03_FPublicación'] < $res[4] 
                || $fila['cGR03_FPublicación'] > $res[5]) {
                        $ind_tratar = 1; 
                }
            } else {
                    if ($res[4] !== $fecha_minima
                    ||  $res[5] !== $fecha_maxima){
                        if ($res[4] == $fecha_minima) {
                            if ($fila['cGR03_FPublicación'] > $res[5]) {
                                $ind_tratar = 1;
                            }
                        } else {
                                if ($fila['cGR03_FPublicación'] < $res[4]) {
                ...
                if ($res[6] !== "0" 
                && $res[6] !== $fila['cGR03_Género']) {
                    $ind_tratar = 1; 
                }
                if ($res[7] !== "0" 
                && $res[7] !== $fila['cGR03_Idioma']) {
                    $ind_tratar = 1; 
                }
                if ($res[8] !== "0" 
                && $res[8] !== $fila['cGR03_Soporte']) {
                    $ind_tratar = 1; 
                }
                if ($res[9] !== "0" 
                && $res[9] !== $fila['cGR03_Estado_S']) {
                    $ind_tratar = 1; 
                }

                if ($res[10] !== $fecha_minima
                &&  $res[11] !== $fecha_maxima){
                    if ($fila['cGR03_FAdquisición'] < $res[10] 
                    || $fila['cGR03_FAdquisición'] > $res[11]) {
                            $ind_tratar = 1; 
                    }
                } else {
                        if ($res[10] !== $fecha_minima
                        ||  $res[11] !== $fecha_maxima) {
                            if ($res[10] == $fecha_minima) {
                                if ($fila['cGR03_FAdquisición'] > $res[11]) {
                                    $ind_tratar = 1;
                                }
                            } else {
                                    if ($fila['cGR03_FAdquisición'] < $res[10]) {
